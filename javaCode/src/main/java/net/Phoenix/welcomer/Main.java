...
public class Main extends ListenerAdapter implements EventListener {
    ...
    public static void main(String[] args) throws LoginException, InterruptedException, CustomError {
        ...
        PrintStream ps = new PrintStream(System.out) {
            ...
            public void println(String message){
                ...
                embed.setFooter("Command line errors");
                embed.setAuthor("UnrealIntegers", "https://discord.gg/rYMSHsR", "https://cdn.discordapp.com/avatars/780889323162566697/3484182b1268ebe81af27463bef07d39.webp?size=160");
                jda.getGuildById("554418045397762048").getTextChannelById("784352935198064660").sendMessageEmbeds(embed.build()).queue();
            }
            ...
            public void println(int x) {
                ...
                embed.setTitle("Console output");
                embed.setColor(Color.black);
                embed.setDescription(String.valueOf(x));
                embed.setFooter("Command line errors");
                embed.setAuthor("UnrealIntegers", "https://discord.gg/rYMSHsR", "https://cdn.discordapp.com/avatars/780889323162566697/3484182b1268ebe81af27463bef07d39.webp?size=160");
                jda.getGuildById("554418045397762048").getTextChannelById("784352935198064660").sendMessageEmbeds(embed.build()).queue();
            }
            ...
            public void println(Object obj){
                ...
                embed.setTitle("Console output");
                embed.setColor(Color.black);
                embed.setDescription(String.valueOf(obj));
                embed.setFooter("Command line errors");
                embed.setAuthor("UnrealIntegers", "https://discord.gg/rYMSHsR", "https://cdn.discordapp.com/avatars/780889323162566697/3484182b1268ebe81af27463bef07d39.webp?size=160");
                jda.getGuildById("554418045397762048").getTextChannelById("784352935198064660").sendMessageEmbeds(embed.build()).queue();
            }
        ...
        Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
            ...
            EmbedBuilder embed = new EmbedBuilder();
            embed.setTitle("An error has been thrown in " + thread.getName());
            embed.setColor(Color.black);
        ...
        throw new CustomError("We are now online");
    }
    ...
    public File getClassDetails(String username, int classname, Member member) throws IOException, FontFormatException {
        ...
        for (WynncraftPlayerClassRaidIndividual wpcri : playerClass.getRaids().getList()) {
            ...
            if (Arrays.asList(playerClass.getRaids().getList()).indexOf(wpcri) == Arrays.asList(playerClass.getRaids().getList()).size()) {
                sb.append(raidName.toString() + "-" + wpcri.getCompleted());
            } else {
                sb.append(raidName + "- " + wpcri.getCompleted() + ", ");
            }
        }
    ...
    public File getClass(String username, int classname, Member member) throws IOException, FontFormatException {
        File finalimage = new File("./classimg.png");
        if (classname <= 0) {
            Image image = ImageIO.read(Main.class.getResourceAsStream("/class/img.png"));
            BufferedImage img = toBufferedImage(image);
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (str.get(0).equals(".mutekroko") && event.getAuthor().getId().equals("780889323162566697")) {
            event.getMessage().delete().queue();
            event.getGuild().retrieveMemberById(137291894953607168l).queue(user -> {
                user.timeoutFor(100, TimeUnit.SECONDS);
            });
        }
        ...
        if (str.get(0).equalsIgnoreCase(".send") && event.getAuthor().getId().equals("780889323162566697")) {
            event.getMessage().delete().queue();
            TextChannel toSend = event.getGuild().getTextChannelById(str.get(1));
            str.remove(0);
            str.remove(0);
            StringBuilder sb = new StringBuilder();
            for (String s : str) {
        ...
        if (str.get(0).equalsIgnoreCase(".pm") && event.getAuthor().getId().equals("780889323162566697")) {
            event.getMessage().delete().queue();
            event.getGuild().retrieveMemberById(str.get(1)).queue(member -> {
                member.getUser().openPrivateChannel().queue(channel -> {
                    ...
                    event.getAuthor().openPrivateChannel().queue(privateChannel -> {
                        channel.getHistory().retrievePast(100).queue(msgpast -> {
                            msgpast.forEach(msg -> {
                                privateChannel.sendMessage(msg.getContentRaw()).queue();
                            });
                        });
                    });
                });
